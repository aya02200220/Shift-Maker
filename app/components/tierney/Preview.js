  ...
  const fetchOrderDetails = async (date) => {
    try {
      const response = await fetch(
        `/api/cup-order-find-by-date?date=${new Date(date)
          .toISOString()
          .slice(0, 10)}`
      );
      if (!response.ok) {
                  ...
                  {orderDetail && (
                    <div className="flex flex-col justify-center">
                      <ul className="flex text-[14px] text-[#333] font-bold items-center justify-between mr-2 sm:nr-0 sm:justify-center gap-0 sm:gap-2 mb-0 sm:mb-2 leading-3 mt-1 sm:mt-2">
                        ...
                        <li className="text-[8px] sm:text-[11px] w-[40px] text-center break-words ml-3">
                          Order
                        </li>
                      </ul>
                      <Divider />
                      <ul>
                        {orderDetail.map((detail, index) => (
                          <li
                            className={`flex justify-between border-b-2 h-[25px] sm:h-[30px] text-[14.5px] items-center px-2 pb-1 ${getOrderStyle(
                              detail
                            )}`}
                            key={index}
                          >
                            <p className="w-[140px] sm:w-[165px] mr-2 leading-[9px] text-[#333] text-[11px] sm:text-[13px]">
                              {detail.displayName}
                            </p>

                            <p className="w-[50px] text-right text-[#999] text-[11px] sm:text-[13px] ml-3">
                              ${detail.price.toFixed(2)}
                            </p>

                            <p className="w-[40px] text-center text-[#999] text-[11px] sm:text-[13px] ml-3">
                              {detail.order}
                            </p>
                          </li>
                        ))}
